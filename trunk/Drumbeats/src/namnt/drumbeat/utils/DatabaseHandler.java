...
//		values.put(KEY_PH_NO, contact.getPhoneNumber()); // Contact Phone
	...
	{
		String selectQuery = "select count(*) from Favorites";
		Cursor cursor = myDataBase.rawQuery(selectQuery, null);
//		Cursor cursor = myDataBase.query(TABLE_FAVORITES, new String[] { KEY_FOLDER, KEY_FILE}, KEY_FOLDER + " = ? and " + KEY_FILE + " = ?",
//				new String[] { folder, file }, null, null, null, null);
		if (cursor != null)
...
//		return song;
//	}
	public ArrayList<Song> getFavariteSong() {
		//SQLiteDatabase db = this.getReadableDatabase();
		ArrayList<Song> songList = new ArrayList<Song>();
		String selectQuery = "select folder, file, id from Favorites order by id asc";
		Cursor cursor = myDataBase.rawQuery(selectQuery, null);
		if (cursor != null)
		{
			if (cursor.getCount()>0)
			{
				if (cursor.moveToFirst()) {
					do {
						Song song = new Song();
						song.setID(String.valueOf(cursor.getInt(2)));
	...
	public void updatealloderfavorite(ArrayList<Song> song)
    {    	
    	String[] columns;
    	ContentValues values = new ContentValues();
    	myDataBase.execSQL("Delete from " + TABLE_FAVORITES);
    	for (int i = 0; i < song.size(); i++) {
    		int id = Integer.parseInt(song.get(i).getID());
    		String folder = song.get(i).getFolder();
    		String file = song.get(i).getName();
    		values.put(KEY_FOLDER, folder); 
    		values.put(KEY_FILE, file);
//    		values.put(KEY_PH_NO, contact.getPhoneNumber()); // Contact Phone
    		myDataBase.insert(TABLE_FAVORITES, null, values);
